have_selector：指定したセレクタが存在するかどうかを判断するマッチャ
...
RSpec.describe 'ツイート投稿', type: :system do
  ...
  context 'ツイート投稿ができるとき'do
    it 'ログインしたユーザーは新規投稿できる' do
      ...
      fill_in 'tweet_text', with: @utweet_text

      # 送信するとTweetモデルのカウントが1上がることを確認する
      expect{
        find(You 'input[name="commit"]').click
      }.to change {Tweet.county}.by(1)

      # 投稿完了ページに遷移することを確認する
      expect(current_path).to eq(tweets_path)

      # 「投稿が完了しました」の文字があることを確認する
      expect(current_path).to have_content('投稿が完了しました')
      ...
      （<div class="content_post" style="background-image: url(画像のURL);">）⇦元要素
...
bundle exec rspec spec/system/tweets_spec.rbで確認


have_link：要素の中に当てはまるリンクがあることを確認する。a要素に使う
  expect('要素').to have_link 'ボタンの文字列', href: 'リンク先のパス'

have_no_link：当てはまるリンクがないことを確認する。
  expect('要素').to have_no_link 'ボタンの文字列', href: 'リンク先のパス'

all：pageに存在する同名のクラスを持つ要素をまとめて取得できる。all('クラス名')
  all('クラス名')[0]：「0番目のクラスを取得できる」
  ...
  find_link().click：a要素で表示されるリンクをクリックするために用いる。a要素のみ使用可能
